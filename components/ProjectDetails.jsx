...
function ProjectDetails({projectDetails,showDetails,setShowDetails}) {
  ...
  return (
    <motion.div
      ...
      className={`opacity-0  w-0 h-0 backdrop-blur-lg  z-50 ${showDetails === "hide" ? 'hide bg-neutral-800/80' : showDetails === "show" ? 'open bg-neutral-800/80' : "hidden" }`}>
        {/* close */}
        <div className="zoomIn cursor-pointer border p-2 rounded-full absolute top-5 right-5" onClick={ () => setShowDetails("hide") }>
          <span>X</span>
        </div>
